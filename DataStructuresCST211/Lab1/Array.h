...
void Array<T>::setStartIndex(const int start_index)							//m_start_index setter
{
	...
	//										//create a temp to hold the old array to be deleted
	//T* to_be_deleted = m_array;
	//for (int i = start_index; i < (start_index + m_length); i++)			//length doesn't change
	//{
	//	if (i < m_start_index || i > (m_start_index + m_length) )		//if the current index is out of previous bounds,					
