...
Exception::Exception(Exception&& copy) noexcept								//move constructor
{
	if (this != &copy)
	{
		m_msg = copy.m_msg;
...
Exception& Exception::operator = (const Exception& move)					//copy assignment
{
	*this = move;
	return *this;
}

Exception& Exception::operator = (Exception&& copy) noexcept				//move assignment
{
	if (this != &copy)
	{
		m_msg = copy.m_msg;
...
ostream& operator << (ostream& out, const Exception& except)				// <<operator
{
	out << except.m_msg;
	return out;
}
