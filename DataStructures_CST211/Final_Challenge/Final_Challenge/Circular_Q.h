...
public:
	...
	void Enqueue(const T data)					//the new enqueue function only allows larger data to be input into the queue
	{
		if (isFull())
		...
		{
			// Compare each element with the new data
			for (int i = front; i != back; i = (i + 1) % m_queue.getLength())
			{
				if (data < m_queue[i])
					throw Exception("Error: New data is less than data already in the queue");
			}

			if (data < m_queue[back])
				throw Exception("Error: New data is less than data already in the queue");

			if (back < m_queue.getLength() - 1)
				back++;
			else
				back = 0;
