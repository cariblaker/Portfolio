...
bool test_peek_full_complex()							//tests peek throughout an entire dequeueing of a full queue
{
	...
	for (int i = 0; i < NUM_SIZE; ++i)
	{
		if (test_Q.Peek() != Q_test.Peek() || test_Q.Peek() != NAMES[i] || Q_test.Peek() != NAMES[i])
			pass = false;
		test_Q.Dequeue();
		Q_test.Dequeue();
	}
...
bool test_print_complex()						//tests printing with a full Queue
{
	...
	try
	{
		test_Q.Print();					//should print the list of names
	}
	catch (Exception& exception)
	{
		pass = false;
...
bool test_enqueue_full_Q()						//tests enqueue when Q is full
{
	...
	try {
		test_Q.Enqueue("DunDuhDuhDuhhhh!!!");
	}
	catch (Exception& exception)
	{
		pass = true;
...
bool test_dynamic_ness() {						//tests the dynamic-ness only with ints
	bool pass = true;
	int length = 15;
	int test;
	Queue<int> test_Q{ length };
	...
	cout << "Dynamic-osity-ness test ";

	return pass;
}


bool test_dynamic_ness_complex() {					//this tests the integrity of the data with strings and the dynamic-ness
	bool pass = false;
	int length = 5;
	string test;
	Queue<string> test_Q{ length };

	for (int i = 0; i < length; i++)
		test_Q.Enqueue(NAMES[i]);

	try {
		test_Q.Enqueue(NAMES[10]);
	}
	catch (Exception& exception)
	{
		pass = true;
	...
	cout << "Dynamic-osity-ness complex test ";
...
test_enqueue_full_Q, test_dynamic_ness, test_dynamic_ness_complex };
