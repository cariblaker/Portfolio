...
List<T>::~List()
{
    ...
    else if (traverse->next_ptr == nullptr)                      //if there is one node, delete it
    {
        delete traverse;
        l_head = nullptr;
        l_count--;
    ...
    l_tail = nullptr;
    if (l_count != 0)
        cout << "Error: destructor malfunction. The list is not empty, and should be!" << endl;
}

template <typename T>
List<T>& List<T>::operator=(const List& copy)                //copy assignment operator
{
    Node<T>* travel = copy.l_head;                          //NOT a new node because APPEND creates a new node and this was causing memory leaks
    while (travel != nullptr)
    {
        this->Append(travel->n_data);
        travel = travel->next_ptr;
