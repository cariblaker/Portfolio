...
U Stack<U>::Pop()
{
	...
	T first{ m_stack.First() };				//there are other methods that could be used here, but
	m_stack.Extract(first);					//this should theoretically remove the head...
	return first;
}
...
{
	bool pass = (m_list.isEmpty()) ? true : false;
	return pass;
}
